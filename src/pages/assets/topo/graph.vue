<template>
    <div class="list add_template graph">
        <div class="w100" style="background: #fff; padding: 10px; height: calc(100vh - 90px)">
            <div class="ub w100 relative">
                ...
                <div class="btn-wrap" style="margin-left: auto">
                    <el-button @click="cancel" size="small">{{ mode === 'preview' ? '返回' : '取消' }}</el-button>
                    <el-button v-if="mode === 'add'" @click="goBack" size="small">上一步</el-button>
                    <el-button
                        v-if="mode !== 'preview'"
                        title="自动布局"
                        ...
                        >自动布局</el-button
                    >
                    <el-button v-if="mode === 'add' || mode === 'edit'" @click="saveTopo()" type="primary" size="small"
                        >完成提交</el-button
                    >
                    <el-button v-if="mode === 'preview'" @click="changeToEdit" type="primary" size="small"
                        >编辑</el-button
                    >
                </div>
            </div>
            <div class="w100 form-wrapper" style="color: rgba(0, 0, 0, 0.9); padding-top: 26px">
                <el-form ref="formData" :model="formData" :rules="formDataRules" label-width="100px">
                    <div class="ub" style="width: 100%">
                        <div class="topo-name" v-if="mode === 'preview' || mode === 'add'">{{ formData.name }}</div>
                        <el-form-item
                            label="拓扑图名称："
        ...
        <!--返回-->
        <el-dialog :visible.sync="backDialog" width="380px" custom-class="custom-dialog" :show-close="false">
...
import serverImg from '@/gplot/assets/images/server.png';
import databaseImg from '@/gplot/assets/images/database.png';
import firewallImg from '@/gplot/assets/images/firewall.png';
import clientImg from '@/gplot/assets/images/client.png';
export default {
    name: 'Graph',
    components: {
