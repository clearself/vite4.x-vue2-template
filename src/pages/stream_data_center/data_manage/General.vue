<template>
    <div>
        <div
            class="event event-wrapper attendance-list expert general"
        ...
        >
                    ...
                    <div class="sql-box ub ub-pj">
                        ...
                        <div class="ub ub-pe" style="width: 200px">
                            <Debounce :time="1500" :isDebounce="2">
                                <el-button :loading="loading" size="small" type="primary" @click.native="searchAssets"
                                    >搜 索</el-button
                                >
                            </Debounce>
                            <Debounce :time="1500" :isDebounce="2">
                    ...
                    <div style="position: relative">
                        ...
                        <div v-if="conditionShow" class="condition-area">
                            ...
                            <div class="ub ub-pe footer">
                                <el-button style="margin-right: 10px" size="mini" @click="conditionShow = false"
                                    >取消</el-button
                                >
                                <Debounce :time="1500" :isDebounce="2">
                                    <el-button @click.native="addFilterCondition" size="mini" type="primary">{{
                                        conditionData.some((it) => it.id === this.recordId) ? '编 辑' : '添 加'
                                    }}</el-button>
                                </Debounce>
                            </div>
                        </div>
            ...
            <div class="chart-wrapper log_num">
                <div class="progress-box">
                    <ProgressBar ref="progressBar" :status="barStatus" />
                    <div class="ub chart-title">
                        <div
                            class="table-view ub ub-pc"
                            @click="viewTab = 'tableView'"
                            :class="{ active: viewTab === 'tableView' }"
                        >
                            ...
                            <i class="el-icon-caret-bottom" v-show="!viewChartOpen"  @click="viewChartOpen=true"></i> -->
                                <i
                                    :class="[
                                        'iconfont',
                                        'icon-a-yibiaopan-xiala1',
                                        viewChartOpen ? 'icon-select1' : 'icon-select',
                                    ]"
                                    @click="viewChartOpen = !viewChartOpen"
                ...
                >
                    <div class="chart-num-box" style="width: 200px; margin-top: 15px">
                        <div class="ub ub-ac mb-3">
                            ...
                            >
                                <i
                                    style="color: #00e9ff; font-size: 12px; margin-left: 10px; cursor: pointer"
                                    @click="handleChartRefresh"
                    ...
                    <div class="ub ub-f1" style="height: 140px">
                        ...
                        >
                            <div
                                ...
                                style="height: 100%; color: #e7e7e7; font-size: 16px; cursor: pointer"
                            >
                                暂无数据
                            </div>
                ...
                <el-form :model="highlightForm" ref="highlightForm" :rules="rules" label-position="top">
                    <el-form-item prop="keywords" label-width="90px">
                        ...
                        <el-input
                            size="mini"
                            style="width: 98%"
                            placeholder="请输入高亮词，多个以英文逗号隔开"
                            v-model.trim="highlightForm.keywords"
                        ></el-input>
                        <el-tooltip
                            class="item"
                            effect="dark"
                            content="为优化页面性能，请将高亮词填写详细，修改成功后，需重新搜索"
                            placement="top-start"
                        >
                            <i
                                style="color: #0052d9; margin-left: 4px; font-size: 12px; position: absolute"
                                class="iconfont icon-qiangtishi"
                            ></i>
                        </el-tooltip>
                    </el-form-item>
                </el-form>
...
// saveSearchTask
// saveSearchHistory,
'@/server/data_manage/new_search.js';
import {
    getFieldByAll,
    getUserField,
    saveUserField,
...
export default {
    ...
    data() {
        ...
        return {
            ...
            rules: {
                ...
                srcPort: [{ validator: checkcallFailedFlowId, trigger: 'blur' }],
                desPort: [{ validator: checkcallFailedFlowId, trigger: 'blur' }],
                reportLevel: [
                    {
                        required: true,
                        message: '请选择事件等级',
    ...
    computed: {
        ...
        exportUrl() {
            ...
            return (
                'data_center/manage/syslog/export?type=0&dtArr=' +
                this.get_params.dt +
                '&content=' +
        ...
        exportUrl1() {
            ...
            let endTime = this.sayTimes === null ? '' : this.sayTimes[1];
            return (
                'data_center/manage/syslog/export?type=1&dtArr=' +
                this.get_params.dt +
                '&content=' +
                this.get_params.content +
                '&startTime=' +
    ...
    methods: {
        ...
        saveTask(val) {
            if (val === 1) {
                ...
                this.dateData1 = this.$getsessionStorage('saveDate').title
                    ? this.$getsessionStorage('saveDate').title
                    : this.$getsessionStorage('saveDate').value.join(',');
            }
        },
                        ...
                        paramsData: {
                            ...
                            param: {
                                inputTerm: [
                                    ...
                                    { field: 'endTime', value: this.customTime[1] },
                                ],
                                sqlTerm: this.sqlTermData,
                                filterTerm:
                                    this.conditionData.length > 0 ? this.conditionData.map((it) => it.origin[0]) : [],
                                conditionData: this.conditionData,
                                time: this.$getsessionStorage('temporary').actualTime
                                    ? this.$getsessionStorage('temporary').actualTime
                                    : this.dateData1,
                                timeStatus: this.$getsessionStorage('temporary').timeUnit
                                    ? this.$getsessionStorage('temporary').timeUnit
                                    : '',
                            },
                        ...
                        paramsData: {
                            ...
                            param: {
                                ...
                                filterTerm:
                                    this.conditionData.length > 0 ? this.conditionData.map((it) => it.origin[0]) : [],
                                conditionData: this.conditionData,
                                time: this.$getsessionStorage('temporaryOld').actualTime
                                    ? this.$getsessionStorage('temporaryOld').actualTime
                                    : this.dateData,
                                timeStatus: this.$getsessionStorage('temporaryOld').timeUnit
                                    ? this.$getsessionStorage('temporaryOld').timeUnit
                                    : '',
                            },
        ...
        addFileds(element) {
            ...
            if (
                this.list.find((it) => it.fieldName === 'alarmTime') &&
                this.list.find((it) => it.fieldName === 'message')
            ) {
        ...
        getCurrentDay() {
            let myDate = new Date();
            let day =
                myDate.getFullYear() + '-' + this.twoStr(myDate.getMonth() + 1) + '-' + this.twoStr(myDate.getDate());
            return day;
        },
        getCurrentWeek() {
            let myDate = new Date();
...
.expert ::v-deep .vue-treeselect__placeholder,
...
.fields-ver-line {
    ...
    box-shadow: 0px 0px 4px 0px #00d0ff, 0px 0px 6px 0px #00d0ff;
    border-radius: 1px;
}
